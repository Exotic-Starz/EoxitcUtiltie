...
exports.run = async(client, msg, args) => {
    ...
    if(!args[1]) {
        ...
        .setColor('0xbf00ff')
        .setDescription(`<@${targetID.user.id}> has been muted by ${msg.author} until you unmute them using the \`unmute\` command!`)
        msg.channel.send(confirmation);
 
        var log = new Discord.MessageEmbed()
    .setColor('0xbf00ff')
    .setDescription(`<@${targetID.user.id}> has been **hard-muted** by ${msg.author}. They will not be unmuted until you unmute them manually!`)
    logs.send(log);
    ...
    var log = new Discord.MessageEmbed()
    .setColor('0x05ff4c')
    .setDescription(`<@${targetID.user.id}> has been muted by ${msg.author} for ${ms(ms(args[1]))}.`)
    logs.send(log);
