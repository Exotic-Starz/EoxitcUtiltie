...
client.on('ready', () => {
    ...
    const statusArray = ['Roblox, PLAYING', 'General chat, WATCHING', 'To Music in :musical_note: Music #1 [Rythm 1] , LISTENING', 'With my dog, PLAYING', 'Exotic Starz videos, Watching']
...
client.on('message', async(msg) => {
    ...
    msg.delete();
    msg.channel.send(`${msg.author} , you are not allowed to send long messages in Exotic World!`)
     
    }
 
    if(msg.channel.id === '824326523192475668')
    await msg.delete();
    if(msg.author.bot) return;
    if(msg.content.toLowerCase() === '!verify' && msg.channel.id === '824326523192475668')
    {   
        await msg.delete
...
client.on('guildMemberAdd', member => {
    ...
    function isValidURL(string) {
      ...
      if(isValidURL(testContent) && msg.channel.id !== '811082489015435284811085356044517447811083501814153286') {
        msg.reply('You cannot send links in this channel if you would like to post a link please you this channel: <#811082489015435284>')
        msg.delete()
        ...
        if(Number.isInteger(warnsJSON[msg.author.id].warns / 3)) {
            ...
            .setColor('RED')
            .setDescription(`${msg.member.user.username} has been muted for violating Exotic Worlds rules`)
    ...
    var array = ['bitch' , 'Bitch' , 'B1itch' , 'b1itch' , 'b!tch' , 'B!tch' , 'dick' , 'Dick' , 'd1ck' , 'Dick' , 'd!ck' , 'D!ck' , 'cock' , 'Cock', 'C0ck' , 'cunt' , 'Cunt' , 'cnut' , 'Cnut' , 'Fuck', 'fuck' , 'fuk' , 'Fuk' , 'fag' , 'fagot' , 'Fag' , 'Fagout' , , 'fucker' , 'basterd' , 'Basterd' , 'buthole' , 'Buthole' , 'crap' , 'Crap' , 'asshole' , 'Asshole' , 'nigga' , 'Nigga' , 'nigg' , 'Nigg' , 'sex' , 'Sex' , 'crap' , 'Crap' , 'hoe' , 'Hoe' , 'pervert' , 'Pervert' , 'pedo' , ];
 
        if(array.some(w =>  ` ${msg.content.toLowerCase()} `.includes(` ${w} `))&& msg.channel.id !== '822555268520345640'){
            var emojiGuild = client.guilds.cache.find(guild => guild.name === 'Exotic World')
            var Pepebonk = emojiGuild.emojis.cache.find(emoji => emoji.name === 'animebonk')
   
            msg.delete()
            msg.reply(`${Pepebonk} Exotic World is a family friendly server continuing will result in a mute!`)
